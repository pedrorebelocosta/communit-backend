...
$app->get('/occurrence', function (Request $request, Response $response, $args) {
	...
	foreach($occurrences as $occurrence) {
		$occurrence["is_road_problem"] = $occurrence["is_road_problem"] == 1 ? true : false;
	}
...
$app->get('/occurrence/{id}', function (Request $request, Response $response, $args) {
	...
	$foundOccurrence = $db->occurrence()->select("id, user_id, description, ST_X(geom) as lat, ST_Y(geom) as lng, photo_url, is_road_problem")
										->where("id = ?", $args["id"])
										->limit(1)[$args["id"]];
	$foundOccurrence["is_road_problem"] = $foundOccurrence["is_road_problem"] == 1 ? true : false;
...
$app->post('/occurrence', function (Request $request, Response $response, $args) {
	require_once("db.php");
	$userId = $request->getAttribute('token')["user_id"];
...
$app->put('/occurrence/{id}', function (Request $request, Response $response, $args) {
	...
	if ($foundOccurrence["user_id"] !== $userId) return $response->withStatus(403);
	if (!$foundOccurrence) return $response->withStatus(404);

	$foundOccurrence["geom"] = new NotORM_Literal("Point(?,?)", $reqBody["lat"], $reqBody["lng"]);
	...
	$foundOccurrence["is_road_problem"] = $reqBody["is_road_problem"] ?? $foundOccurence["is_road_problem"];
